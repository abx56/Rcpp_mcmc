...
double llmnl_initial_mcprice(vec const& beta,vec const& y,mat X,
  ...
  vec denom=log(Xbeta*iota); //n*1
    ...
    if (flag_know_state) {
      ...
      vec prob_init = xb_init/2;//sum(xb_init,1); // matrix of choice probabilities
                                              // if xb_init is a vector
      vec vec_with_init = Vec*prob_init.t(); //marginal probabilities
